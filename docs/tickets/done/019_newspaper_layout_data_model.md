...
1. Apply Supabase migration:
   - Add `layout_config JSONB NOT NULL DEFAULT '{"format":"A4","fill_direction":"column"}'` to `newspapers`
   - Add `grid_col INTEGER`, `grid_row INTEGER`, `col_span INTEGER NOT NULL DEFAULT 1`, `row_span INTEGER NOT NULL DEFAULT 1` to `newspaper_blocks`
   - Backfill existing blocks: column-first A4 layout — `grid_col = (sort_order / 4) % 2`, `grid_row = sort_order % 4`
   - Add CHECK constraints (nullable grid_col/grid_row allowed)

2. Create `src/modules/newspaper/lib/types.ts` with `PaperFormat`, `FillDirection`, `LayoutConfig`, `GridPosition` types

3. Update `updateNewspaper` action to accept and persist `layout_config` (format + fill_direction from formData)

4. Add grid auto-fill helpers to actions.ts: `buildOccupancyGrid` + `findNextAvailableCell`

5. Update `addBlock` action to auto-place blocks using the fill algorithm (fetch newspaper's layout_config + existing block positions)

6. Update `updateBlock` action to persist grid position changes (grid_col, grid_row, col_span, row_span from formData)

## Acceptance Criteria
- [x] Migration applies cleanly on production
- [x] Existing newspapers retain their blocks in correct visual order after backfill
- [x] TypeScript types regenerated and updated
- [x] `updateNewspaper` can change format and fill_direction
- [x] `addBlock` auto-places new blocks without overlap
- [x] CHECK constraints prevent invalid positions

## Summary

Added grid-based layout data model to the newspaper feature. Migration `add_newspaper_layout_config` adds `layout_config` JSONB to `newspapers` (format A4/A5, fill_direction column/row) and `grid_col`, `grid_row`, `col_span`, `row_span` columns to `newspaper_blocks` with CHECK constraints. Existing blocks backfilled using column-first A4 ordering. Created `src/modules/newspaper/lib/types.ts` with `LayoutConfig`, `GridPosition`, `PaperFormat`, `FillDirection` types. Updated `updateNewspaper` to persist layout config, `addBlock` to auto-place via occupancy grid algorithm, and `updateBlock` to accept grid position updates.
